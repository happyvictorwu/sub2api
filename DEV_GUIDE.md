...
| 项目 | 说明 |
|------|------|
| **上游仓库** | happyvictorwu/sub2api |
| **Fork 仓库** | bayma888/slothwatching-bmai |
...
| 数据库凭据 | user=`slothwatching`, password=`slothwatching`, dbname=`slothwatching` |
...
psql -U slothwatching -h 127.0.0.1 -d slothwatching -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER slothwatching WITH PASSWORD 'slothwatching';
...
```bash
# 连接数据库
psql -U slothwatching -h 127.0.0.1 -d slothwatching
...
# 执行 SQL 文件
psql -U slothwatching -h 127.0.0.1 -d slothwatching -f migration.sql
```
...
- [上游仓库](https://github.com/happyvictorwu/sub2api)
