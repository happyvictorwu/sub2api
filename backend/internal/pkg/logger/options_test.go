...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/slothwatching-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/slothwatching-data", "logs", "slothwatching.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "slothwatching.log")
