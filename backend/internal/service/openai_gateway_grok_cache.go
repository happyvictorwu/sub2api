...
const (
	...
	grokClientToolCacheOptInHeader   = "X-Slothwatching-Grok-Client-Tool-Cache"
...
func resolveGrokCacheIdentity(c *gin.Context, body []byte, explicitKey, upstreamModel string) string {
	...
	// upstream session identifiers derived by slothwatching.
...
// the cache-capable route so an intermediate slothwatching does not need to preserve
...
// slothwatching header is consumed locally because buildGrokResponsesRequest only
