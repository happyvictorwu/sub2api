...
const (
	...
	githubRepo     = "happyvictorwu/sub2api"
...
func (s *UpdateService) applyReleaseAssets(ctx context.Context, releaseAssets []Asset) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".slothwatching-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "slothwatching")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "slothwatching" || baseName == "slothwatching.exe" {
