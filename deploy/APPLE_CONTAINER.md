...
Slothwatching can run as a native three-service stack with Apple's `container` CLI. This workflow runs the published Slothwatching, PostgreSQL, and Redis OCI images without Docker Desktop or a Docker-compatible daemon.
...
# Creates volumes/network/containers, waits for dependencies, and starts Slothwatching.
...
# Restart PostgreSQL, Redis, and Slothwatching in dependency order.
...
```bash
export SUB2API_ENV_FILE=/absolute/path/to/slothwatching.env
./apple-container.sh init
./apple-container.sh up
```
...
```dotenv
APPLE_CONTAINER_SUB2API_IMAGE=slothwatching:latest
...
|---|---|
| Application and gateway variables | Passed to Slothwatching from `.env` |
...
| `REDIS_PASSWORD` | Applied to Redis and Slothwatching |
...
The script creates only resources carrying the `org.slothwatching.stack=apple-container` label:

| Type | Names |
|---|---|
| Containers | `slothwatching-apple`, `slothwatching-apple-postgres`, `slothwatching-apple-redis` |
| Network | `slothwatching-apple` |
| Volumes | `slothwatching-apple-data`, `slothwatching-apple-postgres-data`, `slothwatching-apple-redis-data` |

The PostgreSQL volume is mounted at `/var/lib/postgresql`, retaining PostgreSQL 18's default child data directory. Slothwatching and Redis also store data in child directories below their Apple volume mount points. This is required because Apple named volumes do not have Docker's copy-up and mount-point ownership behavior.

## Networking

Apple `container` 1.1 does not provide Compose-style network-scoped service aliases. After PostgreSQL and Redis start, the script reads their current private-network IPv4 addresses from `container inspect`, injects those addresses into a newly created application container, and then starts Slothwatching. The script does not modify `~/.config/container/config.toml` or the macOS host resolver.

All three services attach only to the private `slothwatching-apple` network. Only the application publishes a host port; database and Redis ports remain unpublished.

The application container is intentionally recreated by every `up` and `restart` operation because dependency VM addresses can change after they stop. Application data remains in `slothwatching-apple-data`.
...
container exec slothwatching-apple sh -c \
  'PGPASSWORD="$DATABASE_PASSWORD" pg_dump -h "$DATABASE_HOST" -U "$DATABASE_USER" "$DATABASE_DBNAME"' \
  > backups/slothwatching.sql

# Application configuration and local files.
container exec slothwatching-apple sh -c 'tar -C "$DATA_DIR" -czf - .' \
  > backups/slothwatching-data.tar.gz
...
container delete slothwatching-apple
SUB2API_IMAGE=slothwatching:latest # Match APPLE_CONTAINER_SUB2API_IMAGE in .env.
container run --rm --name slothwatching-apple-data-restore \
  --entrypoint /bin/sh \
  --volume slothwatching-apple-data:/restore \
  --volume "$PWD/backups:/backup:ro" \
  "$SUB2API_IMAGE" \
  -c 'rm -rf /restore/data && mkdir -p /restore/data && tar -xzf /backup/slothwatching-data.tar.gz -C /restore/data'

# Restore the logical database while the application is absent.
container start slothwatching-apple-postgres
until container exec slothwatching-apple-postgres sh -c 'pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB"'; do sleep 1; done
container copy backups/slothwatching.sql slothwatching-apple-postgres:/tmp/slothwatching.sql
container exec slothwatching-apple-postgres sh -c '
  export PGPASSWORD="$POSTGRES_PASSWORD"
  dropdb -h 127.0.0.1 -U "$POSTGRES_USER" --if-exists --force "$POSTGRES_DB"
  createdb -h 127.0.0.1 -U "$POSTGRES_USER" "$POSTGRES_DB"
  psql -h 127.0.0.1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -f /tmp/slothwatching.sql
  rm /tmp/slothwatching.sql
'
...
- The script targets native `linux/arm64` images. The normal Slothwatching release publishes an arm64 variant.
