...
Slothwatching supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to Slothwatching. An explicit empty list
...
```nginx
limit_conn_zone $binary_remote_addr zone=slothwatching_conn:20m;
limit_req_zone  $binary_remote_addr zone=slothwatching_auth:20m rate=5r/s;
limit_req_zone  $binary_remote_addr zone=slothwatching_api:40m rate=30r/s;
...
server {
    ...
    limit_conn slothwatching_conn 40;

    location ~ ^/(auth|api/auth)/ {
        limit_req zone=slothwatching_auth burst=10 nodelay;
        proxy_pass http://127.0.0.1:8080;
    }

    location ~ ^/(v1/)?(embeddings|alpha/search)$ {
        client_max_body_size 32m;
        limit_req zone=slothwatching_api burst=60 nodelay;
        proxy_pass http://127.0.0.1:8080;
    }
...
`gzip_types` and do not use `gzip_types *` for Slothwatching. The
...
Slothwatching `server.trusted_proxies` with the Caddy address/private subnet so the
