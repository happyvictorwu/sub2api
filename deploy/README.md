# Slothwatching Deployment Files

This directory contains files for deploying Slothwatching on Linux servers and Apple-silicon Macs.
...
| `slothwatching.service` | Systemd service unit file |
| `slothwatching-datamanagementd.service` | datamanagementd systemd service unit file |
...
Apple-silicon Macs running macOS 26 can run the complete Slothwatching, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
curl -sSL https://raw.githubusercontent.com/happyvictorwu/sub2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs slothwatching | grep "admin password"
   ```
...
# Restart Slothwatching only
docker compose -f docker-compose.local.yml restart slothwatching
...
# Restart Slothwatching only
docker compose restart slothwatching
...
| `ADMIN_EMAIL` | No | `admin@slothwatching.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf slothwatching-complete.tar.gz deployment/

# Transfer to new server
scp slothwatching-complete.tar.gz user@new-server:/path/to/destination/
...
Slothwatching supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/happyvictorwu/sub2api/releases)
2. Extract and copy the binary to `/opt/slothwatching/`
3. Copy `slothwatching.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
# Stop the service
sudo systemctl stop slothwatching
...
1. Edit the systemd service:
   ```bash
   sudo systemctl edit slothwatching
   ```
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/slothwatching.service
   ```
...
The main config file is at `/etc/slothwatching/config.yaml` (created by Setup Wizard).
...
```
/opt/slothwatching/
├── slothwatching              # Main binary
├── slothwatching.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 slothwatching
...
# View detailed logs
docker compose logs --tail=100 slothwatching
...
# View recent logs
sudo journalctl -u slothwatching -n 50

# Check config file
sudo cat /etc/slothwatching/config.yaml
...
Slothwatching supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).
