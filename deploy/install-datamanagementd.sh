...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/slothwatching/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/slothwatching"
DATA_DIR="/var/lib/slothwatching/datamanagement"
SERVICE_FILE_NAME="slothwatching-datamanagementd.service"
...
参数:
  ...
  --source  指定 slothwatching 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/slothwatching-src
EOF
}
...
if ! id slothwatching >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 slothwatching..."
  useradd --system --no-create-home --shell /usr/sbin/nologin slothwatching
else
  echo "[2/6] 系统用户 slothwatching 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R slothwatching:slothwatching /var/lib/slothwatching
chmod 0750 "$DATA_DIR"
...
systemctl enable --now slothwatching-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status slothwatching-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u slothwatching-datamanagementd -f
2. 在 slothwatching（容器部署时）挂载 socket:
