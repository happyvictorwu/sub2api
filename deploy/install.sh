#!/bin/bash
#
# Slothwatching Installation Script
# Slothwatching 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/happyvictorwu/sub2api/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="happyvictorwu/sub2api"
INSTALL_DIR="/opt/slothwatching"
SERVICE_NAME="slothwatching"
SERVICE_USER="slothwatching"
CONFIG_DIR="/etc/slothwatching"
...
declare -A MSG_ZH=(
    ...
    # Installation
    ["install_title"]="Slothwatching 安装脚本"
    ...
    # Completion
    ["install_complete"]="Slothwatching 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 Slothwatching 服务："
    ...
    ["not_installed"]="Slothwatching 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 Slothwatching。"
    ...
    ["uninstall_complete"]="Slothwatching 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 Slothwatching"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 Slothwatching"
    ...
    ["server_config_desc"]="配置 Slothwatching 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="Slothwatching Installation Script"
    ...
    # Completion
    ["install_complete"]="Slothwatching installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start Slothwatching service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading Slothwatching..."
    ...
    ["not_installed"]="Slothwatching is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove Slothwatching from your system."
    ...
    ["uninstall_complete"]="Slothwatching has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install Slothwatching"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove Slothwatching"
    ...
    ["server_config_desc"]="Configure Slothwatching server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/slothwatching" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/slothwatching" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="slothwatching_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/slothwatching" "$INSTALL_DIR/slothwatching"
    chmod +x "$INSTALL_DIR/slothwatching"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/slothwatching"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/slothwatching.service << EOF
[Unit]
Description=Slothwatching - AI API Gateway Platform
Documentation=https://github.com/happyvictorwu/sub2api
...
User=slothwatching
Group=slothwatching
WorkingDirectory=/opt/slothwatching
ExecStart=/opt/slothwatching/slothwatching
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=slothwatching
...
ProtectHome=true
PrivateTmp=true
ReadWritePaths=/opt/slothwatching
...
start_service() {
    ...
    if systemctl start slothwatching; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u slothwatching -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable slothwatching 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status slothwatching"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u slothwatching -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart slothwatching"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop slothwatching"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if Slothwatching is installed
    if [ ! -f "$INSTALL_DIR/slothwatching" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/slothwatching" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet slothwatching; then
        print_info "$(msg 'stopping_service')"
        systemctl stop slothwatching
    fi

    # Backup current binary
    cp "$INSTALL_DIR/slothwatching" "$INSTALL_DIR/slothwatching.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/slothwatching.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/slothwatching"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start slothwatching

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: Slothwatching must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/slothwatching" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="slothwatching.backup.${current_version}"
        else
            backup_name="slothwatching.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/slothwatching" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop slothwatching 2>/dev/null || true
    systemctl disable slothwatching 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/slothwatching.service
    systemctl daemon-reload
