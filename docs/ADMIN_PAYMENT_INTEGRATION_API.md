...
### 目标
本文档用于对接外部支付系统（如 `slothwatchingpay`）与 Slothwatching 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
```json
{
  ...
  "value": 100.0,
  "user_id": 123,
  "notes": "slothwatchingpay order: cm1234567890"
}
```
...
当 Slothwatching 打开 `purchase_subscription_url` 或用户侧自定义页面 iframe URL 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
...
### 6) `doc_url` 配置建议
- 查看链接：`https://github.com/happyvictorwu/sub2api/blob/main/ADMIN_PAYMENT_INTEGRATION_API.md`
- 下载链接：`https://raw.githubusercontent.com/happyvictorwu/sub2api/main/ADMIN_PAYMENT_INTEGRATION_API.md`
...
### Purpose
This document describes the minimal Slothwatching Admin API surface for external payment integrations (for example, `slothwatchingpay`), including:
...
When Slothwatching opens `purchase_subscription_url` or a user-facing custom page iframe URL, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
...
- View URL: `https://github.com/happyvictorwu/sub2api/blob/main/ADMIN_PAYMENT_INTEGRATION_API.md`
- Download URL: `https://raw.githubusercontent.com/happyvictorwu/sub2api/main/ADMIN_PAYMENT_INTEGRATION_API.md`
