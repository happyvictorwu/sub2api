...
Slothwatching Batch Image MVP provides asynchronous Gemini image batch generation through a unified API surface backed by Redis workers, PostgreSQL state, and provider-specific batch backends.
...
API users do not see Gemini file names, Vertex job names, GCS paths, signed URLs, API keys, or service account material. Downloads are proxied through Slothwatching in this MVP.
...
- Per batch job: up to 1000 reference image attachments total after `output_count` expansion across all items. This is an internal Slothwatching guardrail for request size and cost control, not the generated-image cap and not a Pro Image per-item capability. The generated-output cap is 200 images per job.
...
Operators must enable Gemini/Vertex capability in Google's official console before turning on Slothwatching batch image for any group. Slothwatching feature flags and group switches do not create Google-side access by themselves.

Recommended production path:

- Use a Google Cloud project with billing enabled.
- Enable the relevant Gemini API / Vertex AI APIs for the project.
- Use a service account or Application Default Credentials for the Slothwatching runtime.
...
- Configure Slothwatching with the project id, location, managed bucket, provider account, model whitelist, and pricing.
...
- The Slothwatching `x-goog-api-key` compatibility header still expects a Slothwatching key, not a plain Google key.
...
- Optional object-storage download offload: persist completed image outputs to an operator-configured object store such as GCS, S3, or R2, then issue short-lived signed download links to users. This would avoid routing large image/ZIP downloads through the Slothwatching server, which is useful for small-bandwidth deployments. Keep it opt-in because it needs extra storage credentials, lifecycle cleanup, signed-URL expiry policy, access auditing, and compatibility with output deletion.
