...
Slothwatching has a built-in payment system that enables user self-service top-up without deploying a separate payment service.
...
- [Migrating from SlothwatchingPay](#migrating-from-slothwatchingpay)
...
## Migrating from SlothwatchingPay

If you previously used [SlothwatchingPay](https://github.com/touwaeriol/sub2apipay) as an external payment system, you can migrate to the built-in payment system:

### Key Differences

| Aspect | SlothwatchingPay | Built-in Payment |
|--------|-----------|-----------------|
| Deployment | Separate service (Next.js + PostgreSQL) | Built into Slothwatching, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in Slothwatching admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Not yet (planned) |
| Order Management | Separate admin interface | Integrated in Slothwatching admin dashboard |

### Migration Steps

1. Enable payment in Slothwatching admin dashboard and configure providers (use the same payment credentials)
2. Update webhook callback URLs to Slothwatching's callback endpoints
3. Verify that new orders are processed correctly via built-in payment
4. Decommission the SlothwatchingPay service

> **Note**: Historical order data from SlothwatchingPay will not be automatically migrated. Keep SlothwatchingPay running for a while to access historical records.
