...
describe('admin channel monitor duplicate API', () => {
  ...
  it('does not reuse an operation key across administrators for the same monitor', async () => {
    ...
    expect(sessionStorage.getItem('slothwatching:admin:channel-monitor-duplicate:7:55')).toBe(
      firstAdminHeaders['Idempotency-Key']
    )
    expect(sessionStorage.getItem('slothwatching:admin:channel-monitor-duplicate:8:55')).toBeNull()
  })
