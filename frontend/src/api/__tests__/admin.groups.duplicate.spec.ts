...
describe('admin group duplicate API', () => {
  ...
  it('does not reuse an operation key across administrators for the same group', async () => {
    ...
    expect(sessionStorage.getItem('slothwatching:admin:group-duplicate:7:55')).toBe(
      firstAdminHeaders['Idempotency-Key']
    )
    expect(sessionStorage.getItem('slothwatching:admin:group-duplicate:8:55')).toBeNull()
  })
