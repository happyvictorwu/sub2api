...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready Slothwatching setups.
  ...
  const configContent = `# Grok Build CLI → Slothwatching Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for Slothwatching Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches Slothwatching).
...
description = "Grok 4.5 via Slothwatching (Responses)"
...
# Imagine tools: model IDs go to Slothwatching media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = `# Codex CLI → Slothwatching Grok group
...
[model_providers.slothwatching]
name = "Slothwatching Grok"
base_url = "${baseUrl}"
...
# Grok/Slothwatching path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at Slothwatching OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via Slothwatching'
