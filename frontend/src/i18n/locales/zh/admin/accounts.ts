export default {
    accounts: {
      ...
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 slothwatching 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 slothwatching 端解除该限制。',
      ollamaCloud: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。Slothwatching 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokFreeQuota24hHint: '按 slothwatching 近 24 小时本地 Token 用量估算（上限 {limit}）',
      ...
      grok: {
        ...
        ttsTextLabel: 'TTS 文本',
        ttsTextPlaceholder: '例如：Hello from Slothwatching connectivity test.',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 slothwatching 实例的场景。',
