export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description:
          '配置 Grok CLI、Claude Code、Codex 或 OpenCode，让请求通过当前 Slothwatching Grok 分组发送。文本模型走 Responses；图片/视频使用 Imagine 模型 ID 与媒体端点。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 Slothwatching Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 Slothwatching Grok 分组发送。',
        configTomlHint:
        ...
        codexConfigTomlHint:
          'Codex 官方：wire_api 仅支持 "responses"；优先 env_key，勿与 experimental_bearer_token 混用；非 OpenAI 网关默认 supports_websockets = false（Slothwatching 仍可接客户端 WS 并桥接到 HTTP/SSE）。合并前备份 ~/.codex/config.toml。',
        note:
