...
const FALLBACK_ZH_PHRASE = '我已阅读、理解并同意 Slothwatching 部署与运营合规承诺'
const FALLBACK_EN_PHRASE = 'I have read, understood, and agree to the Slothwatching Deployment and Operation Compliance Commitment'
...
export const useAdminComplianceStore = defineStore('adminCompliance', () => {
  ...
  function requireAcknowledgement(partialStatus?: Partial<AdminComplianceStatus>): void {
    status.value = {
      ...
      document_url_zh: partialStatus?.document_url_zh || status.value?.document_url_zh || 'https://github.com/happyvictorwu/sub2api/blob/main/docs/legal/admin-compliance.zh.md',
      document_url_en: partialStatus?.document_url_en || status.value?.document_url_en || 'https://github.com/happyvictorwu/sub2api/blob/main/docs/legal/admin-compliance.en.md',
