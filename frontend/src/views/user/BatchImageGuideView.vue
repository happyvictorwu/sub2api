...
const PREVIEW_CACHE_DB_NAME = 'slothwatching-batch-image-preview-cache'
...
const endpointBase = computed(() => {
  ...
  return '<你的 Slothwatching API 端点>'
})
...
name: slothwatching-batch-image
  ...
  if (!key) return
	  ...
	  try {
        ...
        items: parsedItems.value,
	      ...
	      `slothwatching-ui-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
	    )
...
async function retryFailedJob(job: BatchImageJobRow | BatchImageJob) {
  ...
  try {
    ...
    const retryJob = await submitBatchImageJob(
      ...
      `slothwatching-ui-retry-${job.id}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
    )
