---
name: slothwatching-admin
description: Manage Slothwatching admin APIs for accounts, redeem codes, groups, proxies, error passthrough rules, TLS fingerprint profiles, imports, exports, batch updates, and raw administrator API calls. Use when the user mentions Slothwatching, admin API keys, account management, redeem code management, recharge codes, invitation codes, bulk account import/export, keeping or deleting accounts, refreshing accounts, clearing errors, CRS sync, or managing Slothwatching backend settings through the admin API.
---
...
node scripts/slothwatching-admin.js accounts list
```
...
node scripts/slothwatching-admin.js accounts bulk-update --ids 40,39 --json '{"concurrency":10}'
node scripts/slothwatching-admin.js redeem-codes list --page-size 20
node scripts/slothwatching-admin.js redeem-codes generate --json '{"count":1,"type":"balance","value":10}' --idempotency-key redeem-$(date +%s)
node scripts/slothwatching-admin.js redeem-codes create-and-redeem --json '{"code":"order_123","type":"balance","value":10,"user_id":123}' --idempotency-key order-123
